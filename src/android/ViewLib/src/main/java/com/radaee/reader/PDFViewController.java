...
public class PDFViewController implements OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    private final RadaeePluginCallback.PDFControllerListener mControllerListner = new RadaeePluginCallback.PDFControllerListener() {
        ...
        // Shared per-page flatten worker. Takes an explicit Document (rather
        // than always reading m_view.PDFGetDoc()) so callers can fall back
        // to a freshly re-opened Document when the live one has already
        // been torn down (e.g. flatAnnots() invoked via the Cordova bridge
        // after the reader Activity started closing).
        private boolean flatAnnotsAtPage(Document document, int page) {
